...
exports.save_logos = async(req,res,next) =>{
    ...
    var now_path = config.BASEURL + filename;
    var new_path = config.BASEURL + filename + "." + filetype;
    file_upload_acition(now_path,new_path,filename+"."+filetype,firstpagesetting,type,(rdata)=>{
        if(!rdata){
            res.json({ status: false });
            return next();
        }else{
...
function file_upload_acition(now_path,new_path,filename,Model,type,callback){
    fs.rename(now_path , new_path,async function(err){
        if(err) {
            callback(false);
        }else{
